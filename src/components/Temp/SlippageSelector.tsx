...
const SlippageSelector = ({expectedSlippage, slippageSelectorOpen, setSlippageSelectorOpen}) => {
	...
	const CellGenerator = ({children, cellIndex, slippageValue}) => {
		if (currentCell == cellIndex) {
			return(
				<ActiveSelectorDropDownCell>{children}</ActiveSelectorDropDownCell>	
			)
		} else {
			return(
				<SelectorDropDownCell onClick={() => {updateSlippage(cellIndex, slippageValue)}}>{children}</SelectorDropDownCell>
			)
		}
	}
		...
		if (currentCell == cellIndex){
			return(
				<ActiveSelectorDropDownCell>
					<InputWrapper>
						<input placeholder="0" defaultValue={swapFormStore.inputs.slippageAmount} onChange={e => onChange(e)} />
						%
					</InputWrapper>
				</ActiveSelectorDropDownCell>	
			)
			...
			return(
				<SelectorDropDownCell onClick={() => {setCurrentCell(cellIndex)}}>Custom %</SelectorDropDownCell>				
			)
		}
	}
	...
	return(
		<Container>
			<SlippageInfo expectedSlippage={expectedSlippage} setSlippageSelectorOpen={setSlippageSelectorOpen} />
			<SelectorDropDown style={{display: slippageSelectorOpen ? 'flex' : 'none'}}>
				<CellGenerator cellIndex="1" slippageValue="0.1" >0.1%</CellGenerator>
				<CellGenerator cellIndex="2" slippageValue="0.5" >0.5%</CellGenerator>
				<CellGenerator cellIndex="3" slippageValue="1.0" >1.0%</CellGenerator>
				<Arrow />
