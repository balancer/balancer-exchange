...
export default class TransactionStore {
  ...
  constructor(rootStore, networkIds: number[]) {
    ...
    networkIds.forEach(networkId => {
      this.allTxRecords.set(networkId, new ObservableMap<string, TransactionRecord>());
    });
  }
    ...
    txRecordHashMap.forEach((value, key) => {
      if (this.isTxPending(value) && this.isStale(value, currentBlock)) {
        lib
          ...
          .then(receipt => {
            this.setTxRecordBlockChecked(networkId, key, currentBlock);
              if (receipt) {
  ...
  @action addTransactionRecord(networkId: number, txHash: string, txResponse: providers.TransactionResponse) {
  ...
  @action setTxRecordBlockChecked(networkId: number, txHash: string, blockNumber: number) {
    const txRecordHashMap = this.safeGetTxRecordHashMap(networkId);
  ...
  @action setTxRecordReceipt(networkId: number, txHash: string, txReceipt: providers.TransactionReceipt) {
    const txRecordHashMap = this.safeGetTxRecordHashMap(networkId);
    const txRecord = this.safeGetTxRecord(networkId, txHash);
  ...
  private safeGetTxRecord(networkId: number, txHash: string): TransactionRecord {
    const txRecordHashMap = this.safeGetTxRecordHashMap(networkId);
