...
async function deployPreConfigured() {
    ...
    const TestToken = new web3.eth.Contract(schema.TestToken.abi, {
        data: schema.TestToken.bytecode,
        gas: MAX_GAS,
        from: defaultAccount,
    });
    const BFactory = new web3.eth.Contract(schema.BFactory.abi, {
        data: schema.BFactory.abi,
        gas: MAX_GAS,
        from: defaultAccount,
    });
    const ExchangeProxy = new web3.eth.Contract(schema.ExchangeProxy.abi, {
        data: schema.ExchangeProxy.abi,
        gas: MAX_GAS,
        from: defaultAccount,
    });
    ...
    for (let i = 0; i < extraTokenParams.length; i++) {
        ...
        const token = await TestToken.deploy({
            data: schema.TestToken.bytecode,
            arguments: [
                extraTokenParams[i].name,
                extraTokenParams[i].symbol,
                18,
                initialSupply,
            ],
        }).send({ gas: MAX_GAS });
    ...
    const proxy = await ExchangeProxy.deploy({
        data: schema.ExchangeProxy.bytecode,
    }).send({ gas: MAX_GAS });
    ...
    for (let i = 0; i <= numPools; i++) {
        ...
        const bpool = new web3.eth.Contract(schema.BPool.abi, poolAddress, {
            from: defaultAccount,
        });
        pools.push(bpool);
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            await tokens[i].methods
                .approve(bpool.options.address, MAX_UINT)
                .send();
            await tokens[i].methods
                .approve(proxy.options.address, MAX_UINT)
                .send();
            console.log(`Binding Token ${i} to BPool...`);
            await bpool.methods
                .bind(
                    tokens[i].options.address,
                    tokenParams[i].balance,
                    tokenParams[i].weight
                )
                .send({ gas: MAX_GAS });
        }
