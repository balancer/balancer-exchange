...
async function test() {
    ...
    const Ti = new web3.eth.Contract(schema.TestToken.abi, addresses.Ti, {
        from: defaultAccount,
    });
    const To = new web3.eth.Contract(schema.TestToken.abi, addresses.To, {
        from: defaultAccount,
    });
    const bPool = new web3.eth.Contract(schema.BPool.abi, addresses.BPool, {
        from: defaultAccount,
    });
    ...
    const LP = new BN(
        '115792089237316195423570985008687907853269984665640564039457584007913129639935000000000000000000'
    );

    let result;

    // MATH
    const UserBalanceI = new BN(
        await Ti.methods.balanceOf(defaultAccount).call()
    );
    const UserBalanceO = new BN(
        await To.methods.balanceOf(defaultAccount).call()
    );
    ...
    const Wi = new BN(
        await bPool.methods.getNormalizedWeight(Ti.options.address).call()
    );
    const Wo = new BN(
        await bPool.methods.getNormalizedWeight(To.options.address).call()
    );
    ...
    try {
        const result = await bPool.methods
            .swap_ExactAmountIn(
                addresses.To,
                Ai.toString(),
                addresses.To,
            ...
            .send({ from: defaultAccount, gas: MAX_GAS });

        console.log('Transaction Result :');
        console.log('------------------');
        console.log(
            'Amount In        : ',
            result.events['LOG_SWAP'].returnValues.amountIn
        );
        console.log(
            'Amount Out       : ',
            result.events['LOG_SWAP'].returnValues.amountOut
        );
        console.log('');
    } catch (e) {
        console.log(result);
    ...
    const UserBalanceI2 = new BN(
        await Ti.methods.balanceOf(defaultAccount).call()
    );
    const UserBalanceO2 = new BN(
        await To.methods.balanceOf(defaultAccount).call()
    );

    const Bi2 = new BN(
        await Ti.methods.balanceOf(bPool.options.address).call()
    );
    const Bo2 = new BN(
        await To.methods.balanceOf(bPool.options.address).call()
    );
